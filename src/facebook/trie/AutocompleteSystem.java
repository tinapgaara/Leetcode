...
public class AutocompleteSystem {
    ...
    public List<String> input(char c) {
        ...
        if (c == '#') {
            insert(root, curstr, 1);
            // clear data
            curstr = "";
            return res;
        }
        String prefix = curstr + c;
        Trie cur = root;
        for (int i = 0 ; i < prefix.length(); i ++) {
            char ch = prefix.charAt(i);
            if (! cur.children.containsKey(ch)) {
                // does not match
                // important !!!!1 need to concat this back to curstr buffer and return
                curstr = prefix;
                return res;
            }
            else {
        ...
        Map<String, Integer> counts = cur.strToCount;
        ChComparator comp = new ChComparator();
        // put this to heap, minheap
    ...
    public void insert(Trie root, String sentence, int count) {
        ...
        for (int i = 0 ; i < sentence.length(); i ++) {
            ...
            cur = cur.children.get(ch);
            Map<String, Integer> map = cur.strToCount;
            if (map.containsKey(sentence)) {
                map.put(sentence, map.get(sentence) + count);
            }
            else {
                map.put(sentence, count);
            }
        }
        cur.isEnd = true;
    }
